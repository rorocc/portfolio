        ...
        <div class="relative md:order-1 order-2">
          ...
          <div class="max-w-prose relative">
            <p class="leading-8 landing-description max-w-prose md:text-left text-center">
              I am interested in <b>joining you for your mission to make movies easier to access</b> for cineastes like me.
              Get to know me and my skills a little better on my portfolio. When you're done here, let's have a coffee!
            </p>
          </div>
        </div>
        <div class="relative w-full md:overflow-x-visible justify-center content-center logo-container md:order-2 order-1">
          <img alt="JustWatch and Chris" class="md:w-80 w-48 m-auto" src="~assets/img/hello/justwatch.png" />
          <a href="#case-studies">
            <Badge class="absolute md:bottom-0 translate-y-1/4 z-10 right-0 md:visible invisible" />
          </a>
        </div>
      </div>
    </section>
...
import Sims from "~/components/pages/index/sims";
import ActionTag from "~/components/structuralComponents/actionTag";
import Badge from "~/components/pages/index/badge";

export default {
  name: 'HelloJustWatch',
  components: {Badge,Sims, ActionTag, WebLink, BobbleHead, Workpiece, CursorChat, CircleDecoration},
      head: {
        title: "JustWatch x Chris | ",
        meta: [
          {
            hid: 'canonical',
            rel: 'canonical',
  ...
  data () {
    return {
      ...
      pos: { x: 0, y: 0 },
      works: [
        { title: 'The Traceable Design System', description: 'An approach to making design decisions more traceable by integrating the concept of design rationale into design systems.', bgColor: '#e5e5e5', textColor: '#000000', imgUrl: 'screen_ds.png', url: '/projects/traceable-design-system', isAvailable: true, pills: ["Desktop", "Prototyping", "Master Thesis"] },
...
.logo-container{
  ...
  .title-gradient{
    background: -webkit-linear-gradient(90deg, #E9D8EC, #E2B6E9);
